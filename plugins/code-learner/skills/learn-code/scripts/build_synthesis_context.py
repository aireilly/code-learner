...
def build_context(base_path: str, max_size: int = 0) -> dict:
    ...
    if max_size > 0 and _context_size(context) > max_size:
        context["summaries"] = [
            s for s in context["summaries"]
            if s.get("analysis_depth") != "api-only"
        ]
...
def main():
    ...
    parser.add_argument("--max-size", type=int, default=80000, help="Max context size in bytes (default: 80000, 0 = no limit)")
