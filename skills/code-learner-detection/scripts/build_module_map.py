...
EXCLUDED_DIRS = {
    ...
    ".pytest_cache", ".tox", ".venv", "venv", "env", ".env", "dist",
    "build", "out", "target", "bin", "obj", ".idea", ".vscode", ".claude",
    ".agent_workspace", "coverage", ".nyc_output", ".next", ".nuxt",
    ".cache", "tmp", "temp", ".ruff_cache", ".vale", ".work",
}
...
def build_module_map(
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        if source_files:
            total_lines = sum(
                count_lines(os.path.join(root, f)) for f in source_files
            )
