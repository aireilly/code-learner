...
If the required CLI tool is not installed, STOP and report:
- GitHub: `"gh CLI not found. Install from https://cli.github.com/"`
- GitLab: `"glab CLI not found. Install from https://gitlab.com/gitlab-org/cli"`
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/fetch_pr_metadata.sh \
  --platform "${PLATFORM}" \
  --pr "${PR_NUMBER}" \
  --repo-path "${REPO_PATH}" \
